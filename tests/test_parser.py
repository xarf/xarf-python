...
from xarf import XARFParser, XARFValidationError, XARFParseError
from xarf.models import MessagingReport, ConnectionReport, ContentReport
...
class TestXARFParser:
    ...
    def test_parse_valid_connection_report(self):
        ...
        assert report.class_ == "connection"
    ...
    def test_unsupported_class_alpha(self):
        """Test unsupported class in alpha version."""
        report_data = {
            "xarf_version": "4.0.0",
            "report_id": "test-id",
            "timestamp": "2024-01-15T10:30:00Z",
            ...
            "source_identifier": "192.0.2.1",
            "class": "vulnerability",  # Not supported in alpha
            "type": "cve",
        ...
        assert report.class_ == "vulnerability"
        errors = parser.get_errors()
        assert len(errors) == 1
        assert "Unsupported class" in errors[0]
